...
def test_data():
    ...
    for i in list_loaders:
        ...
        assert type(data_set.smiles) == list and type(data_set.smiles[0]) == str, 'data.smiles is not a list or the content of list is not a string'
        assert data_set.y.shape[1] == 1, 'data.y is not a column tensor'
        assert data_set.num_graphs == len(data_set.y) == len(data_set.smiles), 'data.x, data.y and data.smiles do not have the same length aka nr of compounds'
